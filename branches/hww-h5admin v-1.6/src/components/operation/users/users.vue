  ...
  <div class="users_wrap" v-loading="d_loading">
      ...
      <template slot="searchBox">
        ...
        <div class="end-3">
          <div class>
            ...
            <el-select v-model="searchData.sex" clearable      ref="select" >
              <el-option
    ...
    <template>
      <el-table :data="tableData" class v-loading="loading">
        ...
        <el-table-column label="绑定设备" min-width="60">
          <template slot-scope="scope">
            <p></p>
          </template>
        </el-table-column>
        <el-table-column label="心知了版本号" min-width="60">
          <template slot-scope="scope">
            <p>{{scope.row.ecgAppVersion}}</p>
          </template>
        </el-table-column>
        <el-table-column label="手机型号" min-width="60">
          <template slot-scope="scope">
            <p>{{scope.row.phoneModel}}</p>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 查询列表
    async get_list(num) {
		if( this.searchData.ageMax < this.searchData.ageMin){

<template>
  <div class="app_banner_wrap" v-loading="loading">
    ...
    <div class="add_box">
      <el-button type="primary" @click="add_fun" v-if="moduleIds.includes(9)">添加Banner</el-button>
    </div>
    <!-- 表单 -->
    <template>
      <el-table :data="tableData" class v-loading="loading">
        ...
        <el-table-column label="图片" min-width="90">
          <template slot-scope="scope">
            <p class="img_box">
              <img @click="enlarge(scope.row.imagPath)" :src="scope.row.imagPath" alt />
            </p>
          </template>
        </el-table-column>
        ...
        <el-table-column label="有效期" min-width="150">
          <template slot-scope="scope">
            <p>{{scope.row.beginDate|f_time}} —— {{scope.row.endDate|f_time}}</p>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="85">
          <template slot-scope="scope">
            ...
            <span
              ...
              v-if="scope.$index!=tableData.length-1&&moduleIds.includes(35)"
            >下移</span>
            <span class="operation" v-if="scope.row.status===2" @click="_View_info(scope.row)">查看</span>
            ...
            >下线</span>
            <span class="operation" v-if="moduleIds.includes(33)" @click="del_item(scope.row.id)">删除</span>
          </template>
        </el-table-column>
      </el-table>
    </template>
        ...
        >
          ...
          <el-form-item prop="imagPath">
            <el-upload
              ...
              :on-success="(response, file, fileList)=>detailsuploadSuc(response, file, fileList)"
          ...
          <div class="line-lwl">
            <el-form-item>
              ...
              <el-button type="success" @click="review_fun" v-if="add_data.type===3">审核通过</el-button>
              <el-button
                type="success"
                @click="edit_funs('table_data')"
                v-if="add_data.type==9"
              >编辑-提交审核</el-button>
              <el-button @click="cancle" v-if="add_data.type!=8">取消</el-button>
            </el-form-item>
          </div>
        </el-form>
      </template>
